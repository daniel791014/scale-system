...
1. 開啟瀏覽器，前往 Python 官方網站：
   - 網址：https://www.python.org/downloads/
   - 或直接下載：https://www.python.org/downloads/windows/

2. 選擇 **Python 3.8 或以上版本**（建議使用 Python 3.10 或 3.11，較穩定）
   - 點擊「Download Python 3.x.x」按鈕
   - 下載 Windows installer (64-bit) 或 (32-bit)，根據您的平板系統選擇

#### 1.2 安裝 Python

1. 執行下載的安裝程式（例如：`python-3.11.x-amd64.exe`）

2. **重要**：在安裝畫面中，**務必勾選以下選項**：
   - ✅ **「Add Python to PATH」**（將 Python 加入系統路徑）
   - ✅ **「Install launcher for all users」**（為所有使用者安裝啟動器）

3. 選擇「Install Now」進行安裝
   - 或選擇「Customize installation」進行自訂安裝（進階使用者）
...
2. 輸入以下命令檢查 Python 版本：
```bash
python --version
```
   應該會顯示類似：`Python 3.11.x`
...
> **如果出現「'python' 不是內部或外部命令」**：
> - 表示 Python 沒有正確加入 PATH
> - 解決方法：重新安裝 Python，並確保勾選「Add Python to PATH」
> - 或手動將 Python 安裝路徑加入系統環境變數

#### 1.4 安裝必要的套件

在命令提示字元中，執行以下命令安裝所有必要的套件：

```bash
pip install streamlit pandas pyserial openpyxl xlsxwriter
```
...
> **注意**：
> - `sqlite3` 是 Python 標準庫，無需額外安裝
> - 如果遇到安裝錯誤，請確認網路連線正常
> - 如果使用公司網路，可能需要設定代理伺服器
