...
from dialogs import show_end_shift_dialog, show_start_shift_dialog, show_undo_confirm
...
def render_active_line(line_name, cur_s, cur_g, wo_std_map,
    ...
    """渲染活動中的產線界面"""
    # 生成 LOT 號碼
    current_lot = dm.generate_lot_number(line_name, cur_s, cur_g)
    
    st.markdown(f"""
    <div class="shift-card">
        <div class="shift-title">📍 {line_name} 生產監控中</div>
        <div><span class="shift-badge">LOT：{current_lot}</span></div>
    </div>
    ...
    if st.button(f"🏁 結算下班 (End Shift)", key=f"btn_end_{line_name}", use_container_width=True, type="primary"):
    ...
    if not pending.empty:
        ...
        if curr is not None:
            with col_finish_btn:
                ...
                if st.button("🏁 結束當前工單", type="primary", use_container_width=True, key=f"fin_{line_name}"):
                    ...
                    try:
                        ...
                        if update_work_order_status(curr["工單號碼"], "已完成"):
                            st.toast(f"✅ {line_name} 工單已結案！")
                            time.sleep(1)
                            st.rerun()
                        else:
                            st.error("找不到該工單")
            ...
            def render_queue_table_internal(q_view, current_item, line_n):
                ...
                if "客戶名" in latest_queue.columns:
                    ...
                    def get_density(row):
                        ...
                        return ""
                    q_df["密度"] = latest_queue.apply(get_density, axis=1)
                    q_df["規格"] = latest_queue.apply(lambda x: f"{dm.format_size(x['長'])}x{dm.format_size(x['寬'])}x{dm.format_size(x['高'])}", axis=1)
                    ...
                    for note_col in ['備註1', '備註2', '備註3']:
                        if note_col in latest_queue.columns: 
                            ...
                            q_df[note_col] = ""
                    # [關鍵] 使用最新的已完成數量計算進度
                    q_df["進度"] = latest_queue.apply(lambda x: f"{int(x['已完成數量'])} / {int(x['預計數量'])}", axis=1)
                else: 
            ...
            if has_new_log_queue:
                # 有新數據時，使用快速刷新（0.5 秒）
                @st.fragment(run_every=0.3)  # [優化] 從 0.5 秒減少到 0.3 秒，加快刷新
            ...
            # 磅秤控制面板：智能刷新（按鈕解鎖後暫停刷新）
            # 檢查按鈕是否已啟用（在 fragment 外部檢查，利用 refresh_work_orders 的 5 秒刷新來更新判斷）
            auto_held_val_check = st.session_state.get(f"auto_held_val_{line_name}")
            is_manually_locked_check = st.session_state.get(f"lock_{line_name}", False)
            buttons_enabled_check = (auto_held_val_check is not None) and (not is_manually_locked_check)
            
            if buttons_enabled_check:
                # 按鈕已啟用時，使用很慢的刷新（5 秒），幾乎暫停刷新
                @st.fragment(run_every=5.0)
                def scale_control_panel_paused(curr_item, line_n, s_curr, g_curr):
                    render_scale_control_panel(curr_item, line_n, s_curr, g_curr, wo_std_map,
                                              STABLE_TOLERANCE, QUICK_STABLE_TIME, HOLD_RELEASE_DIFF,
                                              RESET_THRESHOLD, NG_MIN, NG_MAX, undo_dialog_key)
                scale_control_panel_paused(curr, line_name, cur_s, cur_g)
            else:
                # 按鈕未啟用時，使用快速刷新（0.5 秒），快速響應
                @st.fragment(run_every=0.5)
                def scale_control_panel_active(curr_item, line_n, s_curr, g_curr):
                    render_scale_control_panel(curr_item, line_n, s_curr, g_curr, wo_std_map,
                                              STABLE_TOLERANCE, QUICK_STABLE_TIME, HOLD_RELEASE_DIFF,
                                              RESET_THRESHOLD, NG_MIN, NG_MAX, undo_dialog_key)
                scale_control_panel_active(curr, line_name, cur_s, cur_g)
...
def render_idle_line(line_name, all_line_statuses):
    ...
    if pending_count > 0:
        ...
        if st.button(f"🚀 開班上工 (Start Shift)", key=f"btn_start_{line_name}", type="primary", use_container_width=True): 
...
def render_record_history(line_n, s_curr, g_curr, wo_std_map, undo_dialog_key):
    ...
    if not pass_all_session.empty:
        for _, row in pass_all_session.iterrows(): 
            total_weight_session += float(wo_std_map.get(row["工單號"], 0))
        ...
        if not session_logs.empty: 
            ...
            if not pass_df.empty:
                ...
                for _, row in pass_df.iterrows(): 
                    weight_val = float(row['實測重']) if pd.notna(row['實測重']) else 0.0
                    html_table += f"<tr><td>{row['序號']}</td><td>{row['時間']}</td><td><strong>{weight_val:.1f}</strong></td></tr>"
    ...
    with h_r:
        ...
        if st.button("↩️ 撤銷上一筆", type="primary", disabled=not has_any_logs, use_container_width=True, key=f"undo_{line_n}"):
...
def render_scale_control_panel(curr_item, line_n, s_curr, g_curr, wo_std_map,
    ...
    try:
        ...
        if not notes_html: 
            notes_html = "<div style='opacity:0.5'>(無特殊備註)</div>"
    except: 
        st.error("❌ 資料庫異常")
        return
    ...
    with col_right:
        ...
        # [修正] 四捨五入到 1 位小數後判定，與資訊卡顯示一致
        display_val_rounded = round(display_val, 1)
        ...
        current_logs = current_logs[pd.to_datetime(current_logs["時間"]).dt.strftime("%Y-%m-%d") == today_str]
        ...
        # 生成 LOT 號碼
        current_lot = dm.generate_lot_number(line_n, s_curr, g_curr)
        ...
        <div class="status-container {'status-pass' if (is_pass_weight and buttons_enabled) else ('status-ng-ready' if (is_ng_weight and buttons_enabled) else 'status-fail')}">
            <div class="status-left-panel"><div class="weight-display">{display_val:.1f}<span style="font-size: 0.3em; margin-left: 15px;">kg</span></div></div>
            <div class="status-right-panel">
                <div class="info-box"><div class="info-label">班別 / 組別</div><div class="info-value">{s_curr}-{g_curr}</div></div>
        ...
        with b_l:
            ...
            st.button("紀錄良品\n(PASS)", disabled=btn_pass_disabled, type="primary", use_container_width=True, on_click=do_pass, key=f"btn_pass_{line_n}")
